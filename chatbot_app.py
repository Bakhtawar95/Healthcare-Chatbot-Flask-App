    ...
    def recurse(node, depth):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            if count < len(sym_list):
                ...
                c=calc_condition(syms_exp,num_days)
                if(present_disease[0]==second_prediction[0]):
                    precution_list=precautionDictionary[present_disease[0]]
                    for  i,j in enumerate(precution_list):
                    ...
                    for  i,j in enumerate(precution_list):
                            pre=pre+str(i+1)+') '+j+'<br/>'
                    return c+'<br/>'+"You may have {}" .format(present_disease[0])+ " or "+ "{}" .format(second_prediction[0])+'<br/>'+ description_list[present_disease[0]]+'<br/>'+description_list[second_prediction[0]]+'<br/>'+'<br/>'+'Take the following measures:'+'<br/>'+pre
